...
import { playSound, setVolume, setFade, resetHasStarted, setGlobalPause } from '@/app/store/soundSlice';
...
export default function FadeTimer() {
  ...
  return (
    <div className="w-full flex flex-col gap-4 items-center">
      ...
      <div className="flex items-center justify-around w-full">
        <input type="number" value={hour} onChange={(e) => setHour(+e.target.value)} className="md:p-2 p-1 w-16 rounded bg-black/10 outline-none focus:outline-none text-foreground text-lg no-spinner" />
        <label className="mx-1">ساعت</label>
        <input type="number" value={min} onChange={(e) => setMin(+e.target.value)} className="md:p-2 p-1 w-16 rounded bg-black/10 outline-none focus:outline-none text-foreground text-lg no-spinner" />
      ...
      <div className="flex w-3/4 justify-between">
        <button onClick={onCancel} className="dark:bg-red-700 dark:text-foreground bg-red-500 text-background md:py-2 py-1 px-4 rounded-sm cursor-pointer hover:opacity-85 duration-300 focus:opacity-85 font-medium">
          انصراف
        </button>
        <button onClick={onSave} className="dark:bg-emerald-700 dark:text-foreground bg-emerald-500 text-background md:py-2 py-1 px-4 rounded-sm cursor-pointer hover:opacity-85 duration-300 focus:opacity-85 font-medium">
          شروع
        </button>
      </div>
    </div>
